...
public class FeatureVODBulsat extends FeatureVOD
{
	...
	protected String getVodItemsUrl()
	{
		...
		return url
		        + "?attr=poster_small,poster_medium,poster_large,poster_small_base64,short_description,release,country,imdb_rating,duration,youtube_trailer_url,cast.director.1.name";
	}
	...
	protected void indexVodItemMetaData(VODItem.MetaData metaData, String[] meta)
	{
		...
		for (int j = 0; j < meta.length; j++)
		{
			...
			else if ("poster_small_base64".equals(key))
				bulsatMetaData.metaPosterSmallBase64 = j;
			else if ("poster_medium_base64".equals(key))
				bulsatMetaData.metaPosterMediumBase64 = j;
			else if ("poster_large_base64".equals(key))
				bulsatMetaData.metaPosterLargeBase64 = j;
